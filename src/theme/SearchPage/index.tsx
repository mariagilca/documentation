  ...
  return (
    <svg
      ...
      <defs>
        ...
        <style>{`.cls-1,.cls-2{fill:var(--rmk-color-search-logo)}.cls-2{fill-rule:evenodd}`}</style>
      </defs>
      <path
        className="cls-2"
...
function SearchPageContent(): ReactNode {
  ...
  const shouldIncludeInstance = useCallback(
    (instanceKey: InstanceGroupKey) =>
      instanceKey === 'other'
        ? allSelected
  ...
  const instanceTags = useMemo(
    () =>
      buildInstanceTags(
        docsSearchVersionsHelpers.allDocsData as Record<
          string,
  ...
  const selectedFacetTags = useMemo(() => {
    ...
    selectedInstances.forEach((instance) => {
      (instanceTags[instance] ?? []).forEach((tag) => tagSet.add(tag));
    });
  ...
  algoliaHelper.on(
    ...
    ({results: {query, hits, page, nbHits, nbPages}}) => {
      ...
      type RawAlgoliaHit = {
        url: string;
        docusaurus_tag?: unknown;
        _highlightResult: {hierarchy: {[key: string]: {value: string}}};
        _snippetResult?: {content?: {value: string}};
      };

      const items = mapGroupedHits<RawAlgoliaHit, SearchResultItem>({
        hits: hits as RawAlgoliaHit[],
        getTag: (hit) => hit.docusaurus_tag,
        getUrl: (hit) => hit.url,
        filterInstance: ({key}) => shouldIncludeInstance(key),
        map: ({item, instance}) => {
          const {
            url,
  ...
  const makeSearch = useEvent((page: number = 0) => {
    algoliaHelper.clearRefinements('docusaurus_tag');
    algoliaHelper.clearRefinements('language');
    ...
    if (contextualSearch) {
      ...
      Object.entries(docsSearchVersionsHelpers.searchVersions).forEach(
        ([pluginId, searchVersion]) => {
          ...
          if (shouldIncludeInstance(pluginId as InstanceGroupKey)) {
    ...
    if (!contextualSearch && selectedFacetTags.length > 0) {
      selectedFacetTags.forEach((tag) => {
  ...
  }, [makeSearch, searchResultState.lastPage, selectedFacetTags, allSelected]);

  const groupedResults = INSTANCE_ORDER.map((groupKey) => {
    const groupItems = searchResultState.items.filter(
      (item) => item.instanceKey === groupKey,
    );
    ...
    return {
      key: groupKey,
      label: groupItems[0]!.instanceLabel,
      items: groupItems,
    };
  }).filter(Boolean) as {
  ...
  const visibleCount = searchResultState.items.length;
  const resultsLabel =
    visibleCount > 0
      ? `${documentsFoundPlural(visibleCount)}${
          ...
          <div>
            {groupedResults.map(({key, label, items}) => (
              <section key={key} className={styles.searchGroupSection}>
                ...
                {items.map(({title, url, summary, breadcrumbs}) => (
                  <article key={url} className={styles.searchResultItem}>
                    <Heading as="h3" className={styles.searchResultItemHeading}>
