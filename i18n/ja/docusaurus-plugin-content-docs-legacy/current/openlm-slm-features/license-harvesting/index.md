...
- idle time license release threshold (Default: 15 min): 指定した時間を超えてアイドルだったライセンスが回収可能になります。

10. **Advanced** パネルを展開して、システムリソースのしきい値を設定します。これらの数値は、監視対象アプリケーションがアイドルかどうかを判断するためのしきい値です。表示される既定値は、OpenLM サポートチームから明示的に指示されない限り変更しないでください。不適切な変更は OpenLM システムの性能に悪影響を与える可能性があります。
