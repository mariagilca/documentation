...
1. 左側メニューで **Applications** をクリックします:  
   ![スクリーンショット: OpenLM Application in Okta Admin Interface の追加](/img/legacy/word-image-41990-1-5.png)
2. **Create App Integration** をクリックします:  
   ![スクリーンショット 2: OpenLM Application in Okta Admin Interface の追加](/img/legacy/word-image-41990-2-5.png)
3. 表示されたポップアップで Sign-in method を **OIDC - OpenID Connect**、Application Type を **Web Application** に設定し、**Next** をクリックします。  
   ![スクリーンショット 3: OpenLM Application in Okta Admin Interface の追加](/img/legacy/word-image-41990-3-5.png)
4. アプリケーション名を付け、次のボックスにチェックします:
   - A. Client Credentials
   - B. Authorization Code
   - C. Implicit (Hybrid)

   ![スクリーンショット 4: OpenLM Application in Okta Admin Interface の追加](/img/legacy/word-image-41990-4-5.png)
5. URI は変更しません。正しい URI は、OpenLM 側で統合をインポートした後に提供されます。下にスクロールして **Save** をクリックします（Step 9）。**Save** をクリックすると次のプロンプトが表示されます:  
   ![スクリーンショット 5: OpenLM Application in Okta Admin Interface の追加](/img/legacy/word-image-41990-5-5.png)  
...
4. 任意の名前（例: Okta）を入力します。  
   ...
   ![スクリーンショット: OpenLM on-premise configuration](/img/legacy/word-image-41990-6-5.png)
5. **Save** をクリックします。  
   ![スクリーンショット 2: OpenLM on-premise configuration](/img/legacy/word-image-41990-7-5.png)
6. OpenLM Identity Service 側でインポートが完了すると、アカウントに紐づいた正しい URI が生成されます。  
   ![スクリーンショット 3: OpenLM on-premise configuration](/img/legacy/word-image-41990-8-5.png)
7. OKTA アカウントに戻り、General setting セクションで **Edit** をクリックします:  
   ![スクリーンショット 4: OpenLM on-premise configuration](/img/legacy/word-image-41990-9-5.png)
8. Login セクションまでスクロールし、OpenLM Identity Service の情報（sign-in と sign-out の redirect URLs を含む）を入力します。**Save** をクリックします。  
   **![スクリーンショット 5: OpenLM on-premise configuration](/img/legacy/word-image-41990-10-5.png)**
9. Identity Service のページに戻り、ログアウトします。これで OKTA でログインできるようになります。  
   ![スクリーンショット 6: OpenLM on-premise configuration](/img/legacy/word-image-41990-11-5.png)
...
1. Cloud Portal インスタンス → Identity&Access Management (IAM) → External Providers → Add Provider に移動します。  
   ![スクリーンショット: OpenLM SLMC](/img/legacy/word-image-41990-12-5.png)
2. Okta アカウントの情報を入力します:  
   A. Client ID  
   B. Client Secret  
   C. Authority（例: dev-12345678.okta.com）。
3. 公開ボタン名を入力します（例: Okta）。  
   ![スクリーンショット 2: OpenLM SLMC](/img/legacy/word-image-41990-13-5.png)
...
5. Admin または Manager ロールのいずれかを選択します。  
   ![スクリーンショット 3: OpenLM SLMC](/img/legacy/word-image-41990-14-5.png)
6. **SAVE** をクリックします。OpenLM Identity&Access Management 側でインポートが完了すると、アカウントに紐づいた正しい URI が生成されます。  
   ![スクリーンショット 4: OpenLM SLMC](/img/legacy/Cloud-portal.png)
7. OKTA アカウントに戻り、General setting セクションで **Edit** をクリックします。
...
9. Cloud Portal に戻ってサインアウトし、次のパターンで再サインインします:  
   ...
   ![スクリーンショット 5: OpenLM SLMC](/img/legacy/word-image-41990-16-5.png)
