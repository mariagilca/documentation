...
Virtual License Manager (VLM) は現在 OpenLM Platform のみに互換対応しており、将来的にオンプレミス版を提供する予定です。クラウドベースのため、OpenLM Cloud Portal の登録アカウントが必須です。
...
3. OpenLM Cloud Portal で Virtual License Manager のプロダクトを有効化します。  
   ![OpenLM Portal で Virtual License Manager 製品を有効化。](/img/legacy/word-image-56627-1.png)
...
- Admin Role のユーザーが Virtual License Manager (VLM) を開くと、次の UI が表示されます:  
  ![Admin ロールのユーザーに表示される Virtual License Manager のインターフェース。](/img/legacy/word-image-56627-2.png)

- Manager Role のユーザーが Virtual License Manager (VLM) を開くと、次の UI が表示されます:  
  ![Manager ロールのユーザーに表示される Virtual License Manager のインターフェース。](/img/legacy/word-image-56627-3.png)
...
3. "Available Features" の数量は Virtual License Manager に割り当て可能な feature 数を示します。各 feature は 1 回だけ Virtual License Manager に割り当て可能です。feature を割り当てると、該当 Physical License Manager の "Available Features" 数が減少します。  
   **![Available Features と Total Features 列を表示する Physical License Managers ページ。](/img/legacy/word-image-56627-4.png)**
...
- 検索フィールドを使って特定の Physical License Manager を簡単に見つけられます。探している名前を入力すると、表が自動的に絞り込まれます。  
  ![Physical License Managers テーブルを名前でフィルタリングする検索フィールド。](/img/legacy/word-image-56627-5.png)
...
- 現在のページやタブの情報が必要な場合は、ヘッダー右側の情報 "i" アイコンをクリックします。該当ページやタブに関する詳細が表示されます。  
  ![現在のタブの詳細を提供するページヘッダーの情報「i」アイコン。](/img/legacy/word-image-56627-6.png)
...
- 列の管理、印刷、エクスポートは Physical License Managers 表と同様に行えます。  
  ![Physical License Manager レコードの末尾にある View アイコン。](/img/legacy/word-image-56627-7.png)![検索フィールドがある Physical License Manager の Features ビュー。](/img/legacy/word-image-56627-8.png)
...
1. Virtual License Manager を作成するには、Physical License Managers タブを選択し、"Create Virtual LM" ボタンをクリックします。  
   ![Create Virtual LM ボタンがある Physical License Managers タブ。](/img/legacy/word-image-56627-9.png)
...
3. 片方または両方の Physical License Managers から必要な features を選択して Virtual License Manager に割り当てたら、"SAVE" をクリックします。操作をキャンセルしたり前のページに戻る場合は、"CANCEL" をクリックします。  
   ![名前・タイプ・説明フィールドがある Create Virtual LM フォーム。](/img/legacy/word-image-56627-10.png)![Physical License Managers から Virtual License Manager に割り当てる機能を選択。](/img/legacy/word-image-56627-11.png)
...
- "Total Features" の数量は、利用可能ライセンスの有無に関係なく Virtual License Manager に紐づく feature の総数を表示し、該当 Virtual License Manager の全 feature を把握できます。  
  ![Available Features と Total Features 列がある Virtual License Managers ページ。](/img/legacy/word-image-56627-12.png)
...
- 現在のページ/タブの情報を確認するには、ヘッダー右側の "i" アイコンをクリックします。ページ名が表示されます。  
  ![検索・列管理・エクスポートオプションがある Virtual License Managers テーブル。](/img/legacy/word-image-56627-13.png)
...
- 列の管理、印刷、エクスポートは Virtual License Manager 表と同様に行えます。  
  ![Virtual License Manager レコードの末尾にある View アイコン。](/img/legacy/word-image-56627-14.png)![検索フィールドがある Virtual License Manager の Features ビュー。](/img/legacy/word-image-56627-15.png)

- Virtual License Manager を削除するには、対象を選択して "Delete" ボタンをクリックします。  
  ![Delete ボタンで Virtual License Manager を削除。](/img/legacy/word-image-56627-16.png)

#### Creating a Virtual Pool

1. Virtual Pool を作成するには、表の一覧から 1 つの Virtual License Manager を選択し、"Create Virtual Pool" ボタンをクリックします。  
   ![Create Virtual Pool ボタンがある Virtual License Managers リスト。](/img/legacy/word-image-56627-17.png)
...
4. 前のページに戻る、または操作をキャンセルする場合は "CANCEL" をクリックします。  
   ![名前・割り当てマネージャー・説明・機能数量がある Create Virtual Pool フォーム。](/img/legacy/word-image-56627-18.png)
...
7. Features 表示を閉じるには、クロスクローズアイコンをクリックします。列の管理、印刷、エクスポートは Virtual Pool 表と同様に行えます。  
   ![左タブで新しく作成したプールが選択された Virtual Pools ページ。](/img/legacy/word-image-56627-19.png)![View アイコンで開いた Virtual Pool の Features ビュー。](/img/legacy/word-image-56627-20.png)

- Virtual Pool を削除するには、対象を選択して "Delete" ボタンをクリックします。  
  ![Delete ボタンで Virtual Pool を削除。](/img/legacy/word-image-56627-21.png)

#### Users

1. Manager Role のユーザーを表示するには "Users" タブを選択します。  
   ![Manager ロールを持つユーザーを一覧表示する Users タブ。](/img/legacy/word-image-56627-22.png)
2. OpenLM Virtual License Manager の Manager Role ユーザーを作成するには、Cloud Portal にログインし、左側タブメニューから "Identity & Access Management(IAM) → Users" を選択して "Invite User" ボタンをクリックします。  
   ![Invite User ボタンがある Cloud Portal の IAM Users ページ。](/img/legacy/word-image-56627-23.png)
...
7. ユーザーがメール内のリンクをクリックしてパスワードを設定すると、"Manager Role" として Virtual License Manager にログインできます（"Viewer" ロールとして Cloud Portal にもアクセスでき、Cloud Portal の製品一覧から Virtual License Manager を開けます）。  
   ![Portal ロールを Viewer に、VLM 製品のロールを Manager に設定する Invite User フォーム。](/img/legacy/word-image-56627-24.png)

### Manager Role user guide

- Manager Role で利用できるタブは Virtual Pools タブです。利用できる機能と操作は次のとおりです:  
  ![Manager ロールのユーザーが利用できる Virtual Pools タブ。](/img/legacy/word-image-56627-25.png)
...
- 表ヘッダー右上の 3 つの点が並んだアイコンを使って、列の順序変更、サイズ変更、削除ができます。同じアイコンで表の印刷や CSV エクスポートも可能です。現在のページやタブの情報を表示するには、ヘッダー右側の情報 "i" アイコンをクリックします。  
  ![列の管理・印刷・エクスポートのための Virtual Pools テーブルヘッダーの三点アイコン。](/img/legacy/word-image-56627-26.png)
...
- Virtual Pool のレコード末尾の "View" アイコンをクリックすると Features を表示できます。Feature 名を検索するには検索フィールドを使用します。Features 表示を閉じるにはクロスクローズアイコンをクリックします。列の管理、印刷、エクスポートは Virtual Pool 表と同様に行えます。  
  ![Virtual Pool レコードの末尾にある View アイコン。](/img/legacy/word-image-56627-27.png)![検索フィールドがある Virtual Pool の Features ビュー。](/img/legacy/word-image-56627-28.png)

#### Managing license allocation

- Manager は表の一覧から 1 つの Virtual Pool を選択し、"Manage Licenses Allocations" ボタンをクリックしてライセンス割り当てを管理できます。  
  ![Manage Licenses Allocations ボタンがある選択された Virtual Pool。](/img/legacy/word-image-56627-29.png)

- "ALLOCATE POOL" ページが表示されます。  
  ![選択された Virtual Pool の Allocate Pool ページ。](/img/legacy/word-image-56627-30.png)
...
- 割り当ての変更: ライセンスを割り当てると "Available" が減り "Device Ids" が増加します。割り当てを解除すると逆になります。  
  ![Quantity・Available・Device Ids 列がある Allocate Pool テーブル。](/img/legacy/word-image-56627-31.png)

- Device または User にライセンスを割り当てるには、表から feature を選択して "Execute Licenses Allocations" をクリックします。  
  ![Allocate Pool ページで機能と Execute Licenses Allocations ボタンを選択。](/img/legacy/word-image-56627-32.png)
...
- 変更の保存: "SAVE" をクリックして変更を適用します。  
  ![Device ID と Description フィールドがあるライセンス割り当てフォーム。](/img/legacy/word-image-56627-33.png)
...
- OpenLM Virtual License Manager と実機/サーバー間には 1〜2 分の同期時間があります。  
  ![割り当て後に Device Ids 数が増加したことを示す Allocate Pool ページ。](/img/legacy/word-image-56627-34.png)

- ライセンス割り当ての解除や Device/User の説明変更を行うには、"Device Ids" 列の数値（リンク）をクリックします。次のページが表示されます:  
  ![割り当ての削除とデバイスまたはユーザーの説明の編集を行う Device Ids ページ。](/img/legacy/word-image-56627-35.png)
...
- ヘッダー右側の情報 "i" アイコンでページ名を表示できます。  
  **![検索・列管理・エクスポートオプションがある Device Ids テーブル。](/img/legacy/word-image-56627-36.png)**

- Device または User の説明を編集するには、該当レコード末尾の "Edit" ボタンをクリックします。説明を変更できるフォームが表示されます:  
  ![デバイスまたはユーザーレコードの末尾にある Edit ボタン。](/img/legacy/word-image-56627-37.png)![デバイスまたはユーザーの説明を編集するフォーム。](/img/legacy/word-image-56627-38.png)
...
- OpenLM Virtual License Manager と実機/サーバー間には 1〜2 分の同期時間があります。  
  ![ライセンス割り当てを削除するためデバイスまたはユーザーレコードの末尾にある Delete ボタン。](/img/legacy/word-image-56627-39.png)![ライセンス割り当てが削除された後の Device Ids ページ。](/img/legacy/word-image-56627-40.png)
