...
6. End User Services Service を再起動し、必要に応じてページを更新します。これで Personal Dashboard からログアウト可能になります。  
   ![ユーザー認証を有効にした後にログアウトオプションが表示された Personal Dashboard。](/img/legacy/word-image-50565-3-1.png)
...
1. **EasyAdmin User Interface** → **Administration** → **Roles** に移動します。Roles ウィンドウが開きます。  
   ![ライセンスサーバーをフィルタリングするロールを作成するための EasyAdmin の Roles ウィンドウ。](/img/legacy/word-image-50565-4-1.png)
2. **Add** をクリックし、ロール名と説明を入力して **Save** をクリックします。
3. **Save** 後に **Resources** タブが有効になります。**Resources** → **Add** をクリックします。
4. 対象のリソースを選択します。  
   ![ライセンスサーバーリソースをロールに追加する Resources タブ。](/img/legacy/word-image-50565-5-1.png)
5. **Role Details** タブに切り替え、**Users → ADD** をクリックします。  
   ![ロールを割り当てるための Users セクションがある Role Details タブ。](/img/legacy/word-image-50565-6-1.png)
6. このロールを割り当てるユーザーを選択します。  
   ![新しいロールを割り当てるユーザーの選択。](/img/legacy/word-image-50565-7-1.png)
7. **Administration** → **Roles** に戻り、**agent\_query\_role** をダブルクリックして **Groups** を開き、OpenLM\_Everyone グループを削除します。  
   ![agent_query_role の Groups タブから OpenLM_Everyone グループを削除。](/img/legacy/word-image-50565-8-1.png)
