...
description: "次の手順に従って、Siemens Polarion License Manager からのライセンス使用状況の監視と統計の取得を行うように OpenLM を設定します。"
sidebar_position: 75
---
...
Siemens Polarion はログファイルの解析で監視します。Broker がログファイルを読み取り、データを集計して OpenLM SLM に送信します。
...
2. **License Managers** → **Add License Manager** に移動します。  
   ![OpenLM Broker の Web UI で Add License Manager を開く。](/img/legacy/word-image-59527-2.png)  
3. ドロップダウンから **Siemens Polarion** を選択し、ログファイルのみを使用するため任意のポート番号（例: 888）を入力して **Add** をクリックします。  
   ![OpenLM Broker の Web UI で Siemens Polarion ライセンスマネージャーを追加。](/img/legacy/word-image-59527-3.png)  
4. **Vendors** タブで **Add Vendor** をクリックし、`Siemens Polarion` を入力して **Continue** をクリックします。  
   ![OpenLM Broker の Vendors タブで Siemens Polarion ベンダーを追加。](/img/legacy/word-image-59527-4.png)  
5. **Log Files** タブに移動します。  
   ![OpenLM Broker の Web UI で Log Files タブを開く。](/img/legacy/word-image-59527-5.png)  
...
2. **Servers pending approval** のアイコンをクリックします。  
   ![EasyAdmin で Servers pending approval アイコンを選択。](/img/legacy/word-image-59527-6.png)  
3. Siemens Polarion のエントリ（**OpenLM Generic** と表示）を選択します。  
4. 必要に応じて表示名を入力し、タイムゾーンを確認します。  
5. **Approve** をクリックします。  
   ![EasyAdmin で OpenLM Generic として表示される Siemens Polarion エントリを承認。](/img/legacy/word-image-59527-7.png)  
...
8. **Save** をクリックします。  
   ![EasyAdmin で Use Broker を有効にして Siemens Polarion ライセンスサーバーを手動で設定。](/img/legacy/word-image-59527-9.png)  
