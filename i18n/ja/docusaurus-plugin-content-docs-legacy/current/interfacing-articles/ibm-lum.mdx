...
- **方法 1:** 最小構成 — IBM LUM と同じマシンに OpenLM SLM をインストール。最も簡単ですが Broker の利点は使えません。  
- **方法 2:** 別マシンに OpenLM SLM と IBM LUM クライアントをインストールし、Server がクライアント経由で問い合わせる。  
- **方法 3:** IBM LUM ライセンスサーバーに OpenLM Broker をインストール。Broker がローカルで問い合わせ、Server に送信します。拒否レポートなどの高度な機能が使えるため **推奨** です。  

---

## 方法 1: OpenLM SLM で IBM LUM を問い合わせ

設定手順:

1. [OpenLM SLM をダウンロードしてインストール](https://www.openlm.com/knowledge-base/install-openlm-server-ht802/)します。  
2. **EasyAdmin** で **Administration → License Manager Servers** に移動します。  
3. **LM Tools** タブを開きます。  
4. **LM Type** から **LUM** を選択します。  
5. `i4blt.exe` を OpenLM SLM のインストールフォルダにコピーします:  
...
1. OpenLM SLM マシンに **IIS** がインストールされていることを確認します。  
2. OpenLM SLM マシンに **IBM LUM クライアント** をインストールします。  
3. IBM LUM ライセンスサーバーに OpenLM SLM を参照登録します。  
...
Broker がライセンス使用状況をローカルで問い合わせ、OpenLM SLM に送信します。Java JRE が必要で、Windows と UNIX/Linux サーバーの両方で動作します。  
...
2. Broker を設定します:  
   ...
   - OpenLM SLM のホスト名とポート（既定: 5015）を入力します。  
