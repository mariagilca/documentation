---
title: License4J
description: "OpenLM は多種多様で増え続けるライセンスマネージャーの監視に対応しています。本記事では、OpenLM を License4J と連携させるための設定方法について説明します。"
sidebar_position: 45
---
...
- 同じ License4J ライセンスサーバーに OpenLM Broker をインストールします。Broker が問い合わせ結果を OpenLM SLM に転送します。  
- ネットワーク上の任意の Windows マシンに OpenLM SLM をインストールします（同一サーバーでも可）。EasyAdmin Web アプリで結果を解析・表示します。  
...
1. [OpenLM SLM をダウンロードしてインストール](https://www.openlm.com/knowledge-base/openlm-server-kb4100/)します。  
2. OpenLM SLM Configuration ツールを開きます。  
...
3. 通信ポートを設定します（既定は **5015**）。OpenLM SLM のポートが受信通信で開いていることを確認します。  
4. OpenLM SLM の設定と一致するライセンスサーバー情報を設定します:  
   - **License Servers** ノードを選択します。  
   - 同じホスト名（例: *license4j_server*）を入力し、**Apply** をクリックします。  
5. ライセンスサーバーのポートを追加します:  
   - **license4j_server** ノードを選択します。  
   - **Add Port** をクリックします。  
   - OpenLM SLM と同じポート番号（例: *16090*）を入力します。  
