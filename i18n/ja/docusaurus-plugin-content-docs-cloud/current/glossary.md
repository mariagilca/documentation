...
License Access Control において、サービスが追跡する 1 台のライセンスサーバー。ホスト、ポート、ライセンスマネージャーの種類、オプションファイルによって識別されます。ルールとスケジュールはアセットに紐づき、1 つのアセットに対して同時に有効なスケジュールは 1 つだけです。
...
指名ユーザーサブスクリプション向けの、Autodesk のクラウドベースライセンスサービス。OpenLM は Autodesk Cloud と連携して割り当てと使用状況を報告し、対応するクラウドライセンスソースへアクセスルールを展開します。
...
アプリケーションがライセンスサーバーにライセンスを要求して受け取る瞬間。ライセンスマネージャーはチェックアウト時にアクセスルールを適用し、License Access Control はこのタイミングでルールを適用します。
...
License Access Control において、アセットのルールをライセンスサーバーへプッシュする操作。オンプレミスのライセンスマネージャーには完全なオプションファイルとしてコンパイルされ、SaaS プラットフォームにはルール単位の API 変更として送信されます。デプロイメントはオンデマンド、スケジュール、またはグループ変更時の自動実行で行われ、各実行はデプロイメント履歴に記録されます。
...
指定したユーザー、グループ、またはホストにフィーチャを拒否するオプションファイルのディレクティブ。License Access Control の Deny Access ルールは EXCLUDE ディレクティブにコンパイルされ、優先度の低いユーザーが予約済みの容量を消費しないようにします。
...
指定したユーザー、グループ、またはホストにフィーチャを付与するオプションファイルのディレクティブ。License Access Control の Grant Access ルールは INCLUDE ディレクティブにコンパイルされ、フィーチャの使用を必要とする人に限定します。
...
ライセンス管理を受動的な監視から、能動的でルールベースの制御へと変える OpenLM の自動化機能。誰がどのフィーチャを使用できるかを、Grant Access、Deny Access、Reserve Seats などのアクションからなる読みやすいルールとして（任意でスケジュール付きで）定義すると、LAC がそれを各ライセンスサーバーへデプロイします。オンプレミスのライセンスマネージャーにはチェックアウト時に適用されるオプションファイルとして、SaaS プラットフォームにはルール単位の API 変更として配信されます。すべてのデプロイメントはディレクトリと照合して検証され、デプロイメント履歴に記録されます。
...
競合時にも容量が保証されるよう、特定のユーザー、グループ、またはホストにライセンスを確保するオプションファイルのディレクティブ。License Access Control の Reserve Seats ルールは RESERVE ディレクティブにコンパイルされ、たとえば上級エンジニアのためにプレミアムシートを確保します。
...
### Schedule (License Access Control)（スケジュール）

*OpenLM コンポーネント*

License Access Control において、一連のルールと、それをライセンスサーバーへデプロイする曜日・時刻をまとめた名前付きの週次プラン。1 つのアセットで同時にアクティブにできるスケジュールは 1 つだけで、スケジュールをデプロイすると、サーバー上の現在のルールセットはそのスケジュールの割り当て済みルールで置き換えられます。
