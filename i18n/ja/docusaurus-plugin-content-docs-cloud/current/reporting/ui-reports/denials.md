...
* **License Pull Tolerance**: 一時的な否認を無視する猶予時間（例: 60 秒）
* **Track True Denials Only**: 有効化すると、未解決の否認のみを記録
* 変更は今後のデータにのみ適用
