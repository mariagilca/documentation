...
**Touch Points Events**は、OpenLMワークステーションエージェントがインストールされているワークステーションから、ユーザーが登録済みURL（サブディレクトリを含む）にどのくらいの頻度でアクセスするかを報告します。ワークステーションエージェントは、アクセスされたURLを自動的に検出し、監視対象の候補として提案します。滞在時間ではなく、ウェブサイトへのアクセス頻度を追跡します。

## 前提条件

- **エージェントアクティビティマネージャー**: すべての対象マシンにワークステーションエージェントをインストールします。
