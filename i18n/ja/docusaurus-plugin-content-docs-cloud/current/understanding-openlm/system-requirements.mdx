...
<Tabs>
<TabItem value="workstation-agent" label="ワークステーションエージェント">
...
### 依存関係
- Linuxワークステーションエージェントは、tar.gzパッケージを使用してCentOSにインストールする場合、`libXScrnSaver`ライブラリが事前にインストールされている必要があります
- ワークステーションエージェントは自己完結型の.NETアプリです（追加のランタイムは不要）

### ネットワーク要件
- ワークステーションエージェントは、以下との通信のためにTCPポート443に接続する必要があります。
  - エージェントハブ
  - OpenLM ID
...
<TabItem value="broker" label="ブローカー">
...
| Windows 7 (32ビット) | テスト済み、完全サポート（ブローカーv4.4のみ） | インストーラーと設定ウィザードを提供 |
| Windows 7 (64ビット) | テスト済み、完全サポート（ブローカーv4.4のみ） | インストーラーと設定ウィザードを提供 |
