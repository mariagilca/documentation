---
title: "ディレクトリ同期（Directory Sync）"
description: "OpenLM Platform ドキュメントの Directory Sync リファレンス。"
sidebar_position: 31
---
...
:::info[アプリでの場所]
OpenLM Platform の**アプリランチャー**（右上のグリッドアイコン）を開き、**Users（ユーザー）→ Directory Sync** を選択します。

**事前準備:** ネットワーク内に Directory Sync Agent（DSA）をインストールし、Identity Service で認証できるよう認可ファイルを生成してから、**Agent Manager** でエージェントを承認します。

**関連:** [アイデンティティ（Identity）](/cloud/openlm-administration/identity) · [ユーザーとグループ (Users and Groups)](/cloud/users/users-and-groups)
:::
