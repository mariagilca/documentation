  ...
  return (
    <div style={{ position: 'relative', paddingBottom: 'calc(55.31746031746032% + 41px)', height: 0, width: '100%' }}>
      <iframe
        ...
        title="Process Managerで新しいAgent Procedureを作成"
        frameBorder="0"
        loading="lazy"
        allowFullScreen
...
<Tabs>
<TabItem value="options" label="オプション" default>
...
<TabItem value="exclusions" label="ユーザー/グループでオフ">

#### 概要

特定のユーザーまたはグループに対して、自動ライセンスハーベスティング（自動プロセスリリース）をオフにします。アイドル時間の監視は全ユーザーに対して継続されます。除外を常時または時間帯指定で適用できます。

#### 事前準備

次の条件を満たしていることを確認してください。

- Process ManagerのUIにアクセスできること。
- **Shadow Licenses** タブの **Consumption policy type** が **Workstation** に設定されていないこと。ユーザー/グループ除外を有効にすると、**Workstation** の消費ポリシータイプは使用できません。

#### ユーザーまたはグループの除外を設定

1. Process Managerで **Process Release** タブを開きます。
...
4. 除外に時間帯指定が必要な場合は、**時間帯指定の除外を設定（任意）** に進みます。
5. **Save** を選択します。

#### 時間帯指定の除外を設定（任意）

1. **Enable time framed user/group exclusions** をオンにします。
2. 開始時刻と終了時刻を `HH:MM` 形式で設定します。
3. 除外を適用する曜日を選択します。
4. まだ保存していない場合は **Save** を選択します。

#### 除外の動作

除外は次のように動作します。

- 除外されたユーザーまたはグループについて、Process Managerは該当する監視プロセスに対して、閾値超過のアイドル通知をLicense Harvesterへ送信しません。
- 時間帯指定の除外が有効な場合、除外は選択した曜日と時間帯にのみ適用されます。
- 時間帯指定の除外が無効な場合、除外は常時適用されます。

#### ルールと制限

次のルールが適用されます。

- 除外を有効にする場合、少なくとも1人のユーザーまたは1つのグループを追加する必要があります。
- 時間帯指定の除外が有効な場合、開始時刻と終了時刻の両方が必要で、終了時刻は開始時刻より後である必要があります。

#### トラブルシューティング

問題が解決しない場合は、次の点を確認してください。

- ユーザーまたはグループの除外をオンにできない場合は、**Shadow Licenses** タブの **Consumption policy type** が **Workstation** に設定されていないことを確認します。
- 変更を保存できない場合は、少なくとも1人のユーザーまたは1つのグループを選択して追加したことを確認します。
