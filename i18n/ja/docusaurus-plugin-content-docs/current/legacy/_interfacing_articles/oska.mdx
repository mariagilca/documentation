...
![Broker interface with log and license files](https://cdn.openlm.com/wp-content/uploads/2021/02/broker-interface-with-log-and-license-files-1.png)
...
2. If auto-detect fails, select the license manager node → click **Add Port**.  
...
5. For **data_inquiry**, enter the full path to the OSKA data inquiry command:  
   ...
   You can download it from: OpenLMConsole.exe
	6.	Click Execute to test. Then click Apply.
	7.	Repeat for the status command.
	8.	Expand Vendors → Add Vendor. Enter OSKA.
	9.	Click Apply (select No if prompted to update commands).
	10.	Restart the Broker.
	11.	(Optional) Configure via Broker Web UI.


  ## Add LM configuration in EasyAdmin
	1.	Open EasyAdmin → Start → Administration → License Manager Servers.
	2.	Select the LM Servers tab → click Add License Manager.
	3.	From Type, select OpenLM_Generic.
	4.	Enter a descriptive Display Name (e.g. My OSKA License Server).
	5.	Set the Time Zone (e.g. Istanbul UTC+3).
	6.	Enter the Hostname and Port (e.g. oska_license_server, 12345).
	7.	Check Use Broker.
	8.	Click Save.
