---
title: OLicense
description: Configure OpenLM to interface with OLicense-Server using the direct or Broker method to monitor license usage and statistics.
sidebar_position: 58
---

OLicense is one of the [license managers supported by OpenLM](https://www.openlm.com/license-manager-capabilities/).  
This guide explains how to configure OpenLM to interface with OLicense-Server to monitor license usage and obtain license statistics.

There are two ways to query OLicense-Server:

- **Direct method**: OpenLM SLM queries the license server remotely.  
- **Broker method**: OpenLM Broker is installed on the OLicense-Server machine, queries locally, and sends data to OpenLM SLM.  

## Query using OpenLM SLM only (direct method)

![OpenLM Server interfacing directly with the license manager](/img/legacy/openlm-server-interfacing-directly-with-the-lm-2.png)  
*Direct query between OpenLM SLM and OLicense*

### Configure EasyAdmin

1. Install OpenLM SLM.  
2. Open **EasyAdmin** → **Start → Administration → License Manager Servers**.  
3. From **LM Tools**, select **OLicense** from the LM type drop-down.  
4. Copy `olixtool.exe` from the machine where OLicense is installed  
   - Default path: `C:\Program Files\optimum\olicense`  
   - Paste it to the OpenLM SLM LM folder  
   - Default path: `C:\Program Files\OpenLM\OpenLM Server\LM`  
5. In EasyAdmin, set the **Full path to the OLicense executable** to the location of the copied `olixtool.exe`.  
6. Select **SAVE**.  

![Configure OLicense in EasyAdmin](/img/legacy/word-image-42315-2.png)  
*EasyAdmin configuration for OLicense*

### Add OLicense server

1. In **EasyAdmin**, go to **Start → Administration → License Manager Servers**.  
2. Select **Add LM**.  
3. Enter a descriptive **Display name** (for example, *OLicense central*).  
4. From **Type**, select **OLicense**.  
5. Configure toggles:  
   - **Use Broker**:  
     - *Off* (default): OpenLM SLM queries OLicense directly.  
     - *On*: OpenLM Broker supplies usage information.  
   - **Is Triad Configuration**: leave *Off*.  
6. Enter the **Hostname** and **Port** of the OLicense-Server. Default port: **80**.  
7. Select the appropriate **Time Zone**.  
8. Select **SAVE**.  

![Adding OLicense server](/img/legacy/word-image-42315-3.png)  
*Adding OLicense server in EasyAdmin*



## Query through OpenLM Broker

OpenLM Broker is optional but recommended. It queries OLicense locally and sends data to OpenLM SLM, ensuring buffered communication during network outages.

![Broker interface with log and license files](/img/legacy/broker-interface-with-log-and-license-files-3.png)  
*OpenLM Broker setup with OLicense*

### OpenLM SLM configuration (with Broker)

Repeat the server configuration steps above, but toggle **Use Broker = On**.  

![Server setup with Broker](/img/legacy/word-image-42315-5.png)  
*EasyAdmin setup with Broker enabled*

### OpenLM Broker configuration

1. [Download](https://www.openlm.com/downloads/) and install the latest OpenLM Broker.  
2. Configure Broker to monitor OLicense on the correct port (default: **80**).  
   ![OLicense port setup](/img/legacy/word-image-42315-6.png)  
   *Port setup for OLicense in Broker*  
3. Match all Broker fields with your EasyAdmin configuration.  
4. In the **Commands** node, set the path to `olixtool.exe`.  
   ![Command path setup](/img/legacy/word-image-42315-7.png)  
   *Setting the command path in Broker*  
5. Test the configuration under **status** and **data_inquiry** by selecting **Execute**.  
6. Select **Apply** and restart Broker.  
   ![Broker restart](/img/legacy/word-image-42315-8.png)  
   *Restarting Broker after configuration*



## Alternative: Manual Broker web UI configuration

1. Install the latest OpenLM Broker.  
2. Open the [Broker Web UI](http://localhost:5090/).  
3. Go to **License Managers**.  
   - If OLicense is detected automatically, confirm and save.  
   - If not, select **Add License Manager** → Select **OLicense** → Enter port (default: 80) → Select **ADD**.  
   ![Add OLicense manually](/img/legacy/word-image-42315-9.png)  
   *Adding OLicense manually in Broker*  
4. In **Commands**, set the path to `olixtool.exe`. Execute to confirm output.  
   ![Executable path](/img/legacy/word-image-42315-10.png)  
   *Defining command path in Broker Web UI*  
5. In **Vendors**, select **Add Vendor** → Enter "OLicense" → Continue.  
6. Select **SAVE**.  
7. Configure OpenLM SLM as described earlier.  



## Verify OLicense configuration in EasyAdmin

1. Open **EasyAdmin** (`http://<OpenLM Server>:5015`).  
2. Go to **Start → Widgets → License Servers**.  
3. Verify OLicense appears in the list.  
   - A **green circle** means the connection is active.  

