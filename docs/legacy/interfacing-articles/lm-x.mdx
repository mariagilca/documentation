...
- **Option 2:** Install OpenLM Broker on the LM-X server to query locally and forward the data to OpenLM SLM.  
...
*Diagram: OpenLM SLM querying LM-X directly*
...
6. Configure redundancy:  
   ...
   - **Use Broker**:  
     - Off (default): OpenLM SLM queries LM-X directly.  
     - On: Data is collected by OpenLM Broker instead.  
7. Select **Save**.  

![LMX server configuration](/img/legacy/word-image-42292-3.png)

OpenLM SLM can now query your LM-X license server and present usage information in EasyAdmin.
...
The OpenLM Broker is optional but recommended. Installed on the LM-X host, it queries usage locally and forwards it to OpenLM SLM. Broker ensures buffered communication in case of network outages or security restrictions.  
...
2. Under **License Managers**, select LM Type **LMX** and input the port. Select **Add**.  
   ![Adding the LM-X license manager in the OpenLM Broker web UI.](/img/legacy/word-image-42292-6.png)  
3. Add the vendor: switch to the **Vendors** tab → **Add Vendor** → enter a name → **Continue**.  
   ![Adding a vendor on the Vendors tab in the OpenLM Broker web UI.](/img/legacy/word-image-42292-8.png)  
4. Select **Save**.  
