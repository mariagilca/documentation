...
- OpenLM Broker on the same License4J license server machine. The Broker forwards query results to the OpenLM SLM.  
- OpenLM SLM on any Windows machine in the network (can be the same server). It parses and presents the results in the EasyAdmin web application.  
...
1. [Download and install OpenLM SLM](https://www.openlm.com/knowledge-base/openlm-server-kb4100/).  
2. Open the OpenLM SLM Configuration tool.  
...
3. Set the communication port. The default is **5015**. Ensure the OpenLM SLM port is open for inbound communication.  
4. Configure the license server details to match the OpenLM SLM settings:  
...
5. Add a port for the license server:  
   ...
   - Select **Add Port**.  
   - Enter the same port number configured in OpenLM SLM (for example, *16090*).  
