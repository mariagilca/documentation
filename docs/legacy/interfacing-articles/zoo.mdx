...
1. **OpenLM SLM v2.x or higher** installed on-premises or on a Cloud account.  
...
- Buffered communication to OpenLM SLM (tolerates network failures).  
...
OpenLM Broker must be installed on the same machine as Zoo ([installation guide](https://www.openlm.com/application-notes-v3-0/installing-openlm-v3-0/broker-comprehensive-installation-guide-an4004b/)) and connected to OpenLM SLM ([configuration guide](../openlm-broker/openlm-broker-configuration)).  

### Automatic configuration

1. In Broker, select **Detect**.  
   ![Detecting the Zoo license manager with the Detect option in OpenLM Broker.](/img/legacy/word-image-42296-2.png)  
...
1. Select **Add Port** and enter the Zoo port (default: **80**).  
...
9. Continue with OpenLM SLM configuration.  
...
2. In **License Manager Servers**, select the *pending approval* link.  
3. Select the Zoo LM server entry → select **Approve**.  

![Approving the pending Zoo license server in EasyAdmin.](/img/legacy/word-image-42296-4.png)  

At this point, Zoo License Manager is added to OpenLM SLM.  

### Manual configuration (EasyAdmin)

If automatic approval fails:  

1. In **License Manager Servers**, select **Add LM**.  
...
![Configuring the Zoo license server manually with Use Broker enabled in EasyAdmin.](/img/legacy/word-image-42296-5.png)  

OpenLM SLM is now configured to monitor Zoo.  
