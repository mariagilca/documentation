---
title: Lloyd's Register Licence Manager (LiMBR)
description: This integration allows OpenLM to monitor Lloyd's Register Licence Manager (LiMBR) for license usage, denials, borrowed licenses, and expiration dates.
sidebar_position: 47
---

This integration allows OpenLM to monitor Lloyd's Register Licence Manager (LiMBR) for license usage, denials, borrowed licenses, and expiration dates.

## Requirements

1. OpenLM SLM v21.x or higher installed on a machine in your organization or on a Cloud account.
2. OpenLM Broker v21.x or higher installed on the same machine as LiMBR.

## Monitoring capabilities and features

| Feature | Resolution | License totals | License usage | Denials reporting | Borrowed license reporting | Expiration date reporting | Multiple server redundancy |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Support | Start time: by second, end time: by minute | ✔ | ✔ | ✔ | ✔ | ✔ | ❌ |

Additionally, using OpenLM Broker allows for buffered communication and the ability to remotely start or stop the LiMBR service.

## OpenLM Broker configuration

![Diagram of how OpenLM interfaces with LiMBR](/img/legacy/word-image-42114-1.png)  
*Diagram of how OpenLM interfaces with LiMBR.*

OpenLM Broker queries LiMBR locally through its command-line utility, compiles the results, and sends the data to OpenLM SLM.

:::note
OpenLM Broker must already be [installed](https://www.openlm.com/application-notes-v3-0/installing-openlm-v3-0/broker-comprehensive-installation-guide-an4004b/) and [configured](https://www.openlm.com/Knowledgebase%20and%20articles/openlm-broker-configuration/) to report to OpenLM SLM before proceeding.
:::

### Automatic configuration

1. In the Broker configuration tool, select **Detect**.
2. If LiMBR is running, Broker creates a port node with the required settings.
3. Select **Apply**.
4. Select **Restart Broker**.
5. To verify, go to **Commands → data_query** and select **Execute**.

![Verifying the LiMBR data_query command output in OpenLM Broker.](/img/legacy/word-image-42114-2.png)

### Manual configuration

1. Select **Add Port**.
2. Enter the LiMBR port number (default: `11362`).
3. Set **License Manager Type** to **LiMBR** and select **Apply**.
4. In **Commands**, browse to the folder where LiMBR is installed, select **Update**, then **Apply**.
5. In **status**, check configuration.
6. In **data_inquiry**, select **Execute** and ensure the output is valid.
7. Select **Restart Broker**.
8. Under **Vendors**, enter **LiMBR** in **Vendor Name** and select **Apply**.
9. Select **Apply** again.
10. Select **Restart Broker**.

![Automatic Broker configuration](/img/legacy/automatic-broker-configuration.png)

## OpenLM SLM configuration

### Automatic configuration through EasyAdmin

1. Open **EasyAdmin** (`http://<OpenLM Server FQDN>:5015` or through **Windows Start → OpenLM → OpenLM EasyAdmin User Interface**).
2. In **License Manager Servers**, approve the new LiMBR entry.

![Approving the new LiMBR entry in the EasyAdmin License Manager Servers list.](/img/legacy/word-image-42114-4.png)  
![Confirming the approved LiMBR license server in EasyAdmin.](/img/legacy/word-image-42114-5.png)

### Manual configuration

1. In EasyAdmin, go to **Administration → License Manager Servers → Add LM**.
2. Set **Type** to **LiMBR**.
3. Enter a display name and description (for example, LiMBR).
4. Set the time zone.
5. Enter the hostname and port (default: `11362`).
6. **Is Triad Configuration**: off.
7. **Use Broker**: on.
8. Select **Save**.

![Configuring the LiMBR license server manually with Use Broker enabled in EasyAdmin.](/img/legacy/word-image-42114-6.png)

## How to verify configuration

1. Open EasyAdmin (`http://<OpenLM Server FQDN>:5015` or `Cloud.openlm.com`).
2. Go to **Start → Widgets → License Manager Servers**.
3. Verify that LiMBR appears with a green status indicator.

![LiMBR showing a green status indicator in the EasyAdmin server list.](/img/legacy/word-image-42114-7.png)

:::note
It may take up to 3 minutes for a new connection to appear as active in EasyAdmin.
:::