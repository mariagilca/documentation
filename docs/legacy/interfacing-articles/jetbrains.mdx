...
1. **OpenLM SLM v5.6 or higher** (or OpenLM Platform)  
...
Broker queries JetBrains FLS locally over its HTTP port and sends the data to OpenLM SLM.  

:::note
Before continuing, ensure Broker is [installed](https://www.openlm.com/application-notes-v3-0/installing-openlm-v3-0/broker-comprehensive-installation-guide-an4004b/) and [configured](https://www.openlm.com/Knowledgebase%20and%20articles/openlm-broker-configuration/) to report to OpenLM SLM.
:::
...
3. From **Type**, select **JetBrains**. Select **Apply**.  
4. In **Commands**, set the **Update path for commands** to the JetBrains FLS root folder. Select **Update → Apply**.  
5. Open **data_inquiry**. Replace `<TOKEN>` with the JetBrains token. Example:  
  ...
  Select **Execute → Apply**.  

6. Under **Vendors**, set **Vendor Name = JetBrains**. Select **Apply**.  
...
5. JetBrains FLS is now added to OpenLM SLM.  
...
1. In **Server Configuration → License Servers**, select **Add**.  
