...
- Install OpenLM Broker on the license server, query locally, and forward results to OpenLM SLM.
...
8. Set **Use Broker**:  
   - **Off** (default): OpenLM SLM queries the license manager directly.  
   - **On**: OpenLM SLM relies on Broker installed on the license server.  
9. Select **Save**.

![Add BetaLM license server](/img/legacy/word-image-42306-3.png)

Your OpenLM SLM is now ready to query BetaLM and display results in EasyAdmin.
...
The OpenLM Broker is optional but recommended. Install it on the BetaLM license server to activate local queries and buffered communication with OpenLM SLM. This prevents data loss during temporary network failures.
...
3. Enter the BetaLM license server port (default: 6007).  
   :::note  
   The port number must exactly match the configuration in OpenLM SLM.  
   :::  
