...
1. OpenLM SLM v2.x on your premises (installed on a machine in your organization) or an OpenLM Platform account
...
1. Open your browser and navigate to [http://localhost:5090](http://localhost:5090)  
   If port 5090 is occupied, use an incremental port (for example, 5091, 5092).
2. Select the **License Managers** tab.
3. Select **Add License Manager**.  
   A pop-up appears.
4. From the **LM Type** drop-down list, select **Canvas**.
5. Enter the Canvas port number (for example, `5093`) and select **Add**.

   ![Adding a Canvas license manager in OpenLM Broker.](/img/legacy/word-image-98722-2.png)  
   *Adding a Canvas license manager in OpenLM Broker*

6. Select the **Vendors** tab. Select **Add Vendor**.  
   In the pop-up, enter `Canvas` as the vendor name and select **Continue**.

   ![Adding the Canvas vendor in OpenLM Broker.](/img/legacy/word-image-98722-3.png)  
   *Adding Canvas vendor in OpenLM Broker*

7. Switch to the **Log Files** tab. Select **Add Log File**.  
   Make sure **Watch file by pattern** is turned **off**.  
   You can add multiple logs. Browse to the location of the activity log, then select **Continue**.

   ![Adding a log file for Canvas in OpenLM Broker.](/img/legacy/word-image-98722-4.png)  
...
2. Select the icon for **Servers Pending Approval**.

   ![Pending license managers in EasyAdmin.](/img/legacy/word-image-98722-5.png)  
   *Pending license managers in EasyAdmin*

3. Double-click the pending license manager.  
   It will be displayed as **OpenLM Generic**.

   ![Pending Canvas license manager shown as OpenLM Generic in EasyAdmin.](/img/legacy/word-image-98722-6.png)  
...
4. (Optional) Modify the display name. Verify the configuration, then select **Approve**.
   ...
   ![Approving a Canvas license manager in EasyAdmin.](/img/legacy/word-image-98722-7.png)  
...
5. (Optional) Switch to the **Custom Fields** tab to add details such as country, usage scope, and description.
   ...
   ![Adding custom fields for a Canvas license manager in EasyAdmin.](/img/legacy/word-image-98722-8.png)  
...
3. Verify that the configured license manager appears in the server list.  
   ...
   ![Green circle indicating an active Canvas connection in EasyAdmin.](/img/legacy/word-image-98722-9.png)  
