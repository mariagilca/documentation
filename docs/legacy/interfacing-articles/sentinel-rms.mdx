...
- Install OpenLM SLM v4.3 or higher on a machine in your organization (or use a Cloud account).
...
Monitoring features depend on whether you use OpenLM SLM alone or together with OpenLM Broker.
...
- Yellow cells: Features available with OpenLM SLM.  
- Blue cells: Extra features available with OpenLM SLM + Broker.  

Using Broker also allows buffered communication and remote start/stop of the license manager service.

## Monitoring with OpenLM SLM only

![Diagram of how OpenLM interfaces with Sentinel RMS using OpenLM Server only](/img/legacy/word-image-42333-6.png)
*Diagram of OpenLM SLM querying Sentinel RMS directly*

OpenLM SLM can query Sentinel RMS directly through RMS command-line tools. This option provides license totals and usage only.

### LM tools configuration

To allow OpenLM SLM to query Sentinel RMS:
...
1. Copy `lsmon.exe`, `lservnt.exe`, and `lsapiw32.dll` from the Sentinel RMS machine (for example,  
   ...
   to the OpenLM SLM installation folder (for example,  
...
When Broker queries Sentinel RMS locally using RMS command-line utilities and log files, you gain full monitoring capabilities. Broker then forwards the data to OpenLM SLM.
...
If Broker was configured successfully, Sentinel RMS is added automatically to OpenLM SLM.  

If automatic setup fails, configure manually:

1. In OpenLM SLM configuration, add Sentinel RMS server.  
...
```text
	...
	7.	Verify in OpenLM SLM that token calculations are enabled.
