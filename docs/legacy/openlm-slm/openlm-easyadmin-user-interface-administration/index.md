...
Note **that if you update the product/feature or feature/package manually through the package/product window and then switch to auto-update,** the previous mapping will be overridden.
...
**Enable detailed logs:** use this to activate detailed logs for the predefined period. It also prevents forgetting the running logs and overloading the system.  
...
When this option is activated, OpenLM SLM will try to translate the workstation's names to IP addresses daily at the selected time.
...
**Set as permanent**  
Selecting this option will make the stored user information anonymously irrevocable. You won't be able to switch it back. Your system will not be able to collect clear user information. Make sure you understand the consequences.  
...
**Show unassigned projects:** Display unassigned projects.

**Support environment variable:** Configure an environment variable for OpenLM use.

**Variable name:** Specify the environment variable's name.

**Add unknown projects:** Add unknown projects to OpenLM based on the value of the environment variable.
...
2. Conditional license retrieval (for example, "close idle application A only if application B is inactive")
...
Run a sync request with ServiceNow outside the regularly scheduled time. While scheduled sync is running, Sync Now is deactivated.

**ServiceNow data aggregation time zone:**  
OpenLM reports daily Aggregated data to ServiceNow. Select the time zone to configure the term day.
...
**Status:**  
License Server status can be pending, activated, deactivated, or denied.
...
Deactivated: The LM server is configured, but monitoring has been turned off.
...
**Compare:** Compare the current version of the license file with a draft version.

![Administration - LIcense File Compare](/img/legacy/Screenshot-2024-04-22-at-15.47.51.png)

Administration - License File Compare

**Push to LM Server:** Upload the chosen license file (original or draft) to the license server.

**Search in file:** Search for specific keywords within the license file.
