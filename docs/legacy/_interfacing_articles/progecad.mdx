...
[ProgeCAD](http://www.progesoft.com/) is a 2D and 3D design software for CAD and concept sketching. It is compatible with AutoCAD and supports Direct Modeling in native `.dwg` format.
...
![Broker interface with log and license files](https://cdn.openlm.com/wp-content/uploads/2021/02/broker-interface-with-log-and-license-files-6.png)
...
![Add license manager in EasyAdmin](https://cdn.openlm.com/wp-content/uploads/2021/02/word-image-42305-2.png)

## Configure OpenLM Broker (desktop UI)

1. Download and install the latest **OpenLM Broker**.  
2. In Broker, click **Add Port**.  
...
3. In **Commands**, select **data_inquiry** and enter:  
    ...
    ```
  4. Click **Execute** to verify the command.  
5. Click **Apply**.  
6. Repeat steps 3–5 for the **Status** command.  
7. In **Vendors**, type **ProgeCAD** (case sensitive) and click **Apply**.  
8. Click **Restart Broker**.

![Broker commands configuration](https://cdn.openlm.com/wp-content/uploads/2021/02/word-image-42305-3.png)
...
4. From **Type**, select **OpenLM Generic**. Enter the port (for example, `51168`) and click **Add**.  

![Add license manager web UI](https://cdn.openlm.com/wp-content/uploads/2021/02/word-image-42305-4.png)
...
6. Click **Save**, then **Execute** to validate the output.  

![Broker web UI commands](https://cdn.openlm.com/wp-content/uploads/2021/02/word-image-42305-5.png)

7. Go to the **Vendors** tab. Click **Add Vendor**. Enter `ProgeCAD` (or another free-text name). Click **Continue → Save**.
...
2. Click the **Pending** icon and approve the ProgeCAD license manager.  
