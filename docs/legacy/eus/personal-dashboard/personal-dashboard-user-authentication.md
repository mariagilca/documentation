...
4. Restart the End User Services Service and, if necessary, refresh the page. We observe that now it is possible to log out from our Personal Dashboard Account:  
   ![Personal Dashboard showing the log out option after user authentication is enabled.](/img/legacy/word-image-50565-3-1.png)
...
1. Access the **EasyAdmin User Interface**→**Administrtation**→**Roles**. The Roles Window opens.  
   ![EasyAdmin Roles window used to create a role for filtering license servers.](/img/legacy/word-image-50565-4-1.png)
2. Select **Add.**Provide a Role name and description. Select **Save.**
3. After Selecting Save, the **Resources** tab is enabled. Select it then → **Add.**
4. Select the desired roles:![Resources tab where license server resources are added to the role.](/img/legacy/word-image-50565-5-1.png)
5. Switch to the **Role Details** tab and select **Users→ADD:**![Role Details tab with the Users section for assigning the role.](/img/legacy/word-image-50565-6-1.png)
6. Select the users to assign this role:  
   ![Selecting the users to assign the new role to.](/img/legacy/word-image-50565-7-1.png)
7. Go back to **Administrtation**→**Roles**and double select **agent\_query\_role**→**Groups**and delete OpenLM\_Everyone group:  
   ![Removing the OpenLM_Everyone group from the agent_query_role Groups tab.](/img/legacy/word-image-50565-8-1.png)
