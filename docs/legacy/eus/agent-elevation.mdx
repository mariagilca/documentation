...
1. **Installation through elevated command prompt** (installer run manually as admin, for example, through `msiexec`):
...
2. **Installation through SCCM (System Account)**:
...
You can toggle elevation in the Workstation Agent installation directory through `permissions.runAsElevated`.
...
### Limitations: Non-Elevated Agent versus Elevated process
...
- **Command line argument parsing:** command-line arguments cannot be read. *Impact:* Applications Manager rules that rely on specific start arguments will not trigger.
...
### Supported features (Non-Elevated Workstation Agent versus Elevated process)
...
- **Process management**, including:
  ...
  - **Process termination:** through Workstation Agent procedure or Applications Manager.
  - **Screenshots of the monitored process** (when activated)
