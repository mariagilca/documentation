...
description: Configure the OpenLM Applications Manager to measure the actual usage and idle time of processes tracked by OpenLM Workstation Agents.
sidebar_position: 8
---
...
2. Navigate to Process Features. To do this select**[Start] > [Administration]** to open the Administration screen, then select the **Process Features** icon (see**Figure 1**). This will open the Administration — Process Features screen. ![The Process Features icon on the EasyAdmin Administration screen.](/img/legacy/word-image-26671-1.png)  
   **Figure 1: Locating the Process Features icon.**
3. Add a process. To do this select the**[Add]** button in the upper right of the screen (see **Figure 2**). This will open the Add Process screen. ![The Add button on the Process Features screen.](/img/legacy/word-image-26671-2.png)  
...
4. Enter the **Process Name**, **Description** and **Vendor Name** fields. Be sure "Enabled" and "Track process idle/active periods" boxes are both checked.  
   *[NOTE: The name of the process should match exactly with the feature to be tracked. Locate process names using Process List (**Start → OpenLM → Process List**). The process must be active/running to appear in the list. The Vendor Name must match the vendor name used when configuring products in the Product List screen (select **Configure** on the OpenLM Applications Manager tab of the Agent Configuration screen).]  
   *![The Add Process screen with Process Name, Description and Vendor Name fields highlighted.](/img/legacy/word-image-26671-3.png)  
   **Figure 3: Highlighted entries for Step #4.**
5. Select**[Save]** to return to the Process Features window. A new row will be added to the upper half of the screen (see **Figure 4**).  
   ![The new process row added to the Process Features screen.](/img/legacy/word-image-26671-4.png)  
   **Figure 4: The new row added to the Process Features screen.**
6. Select the new row to activate it. Several buttons will appear at the lower right of the screen.
7. Select the**[+Add Vendor's Features]** button (see **Figure 5**). A confirmation screen will appear (see **Figure 6**).  
   ![The +Add Vendor's Features button on the Process Features screen.](/img/legacy/word-image-26671-5.png)  
   **Figure 5: The [+Add Vendor Features] button.**  
   **![The Add All Features confirmation screen.](/img/legacy/word-image-26671-6.png)  
   **Figure 6: The Add All Features confirmation screen.**
8. Select**[Yes]**to accept the changes. The feature name(s) associated with the process will populate in the bottom half of the Process Features window. Tracking idle time for features is now set.
...
![The Currently Consumed Licenses screen.](/img/legacy/Screenshot-2023-02-24-at-15.55.57.png)

**Figure 7: The Currently Consumed Licenses screen**

Selecting the **Idle Times** icon ( ![The Idle Times icon.](/img/legacy/word-image-26671-8.png) ) to the right of the row will reveal the **Session Active/Idle Ratio** screen.
