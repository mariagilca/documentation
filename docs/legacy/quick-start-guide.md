...
Before proceeding with the installation, please consult the System Requirements page and make sure you meet them. Keep in mind that OpenLM is a modular software.  
...
- **Workstation Agent** - The OpenLM Workstation Agent shows the real usage and the idle time of the engineering applications. It allows the monitoring of the executables, processes, and URLs. Furthermore, the Agent enables you to identify idle open sessions and safely release them.
...
- **Directory Sync - (DSS and DSA)** allows OpenLM to sync its database with directory services that support the LDAP protocol. The Directory Synchronization Agent (DSA) is the component that executes the sync definitions as received from the DSS.

- **ServiceNow Adapter** - The ServiceNow Adapter enables you to integrate OpenLM data into ServiceNow.
