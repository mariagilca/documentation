...
3. *(optional)* If you select not to install the OpenJDK 11 distributive that comes bundled with the Reports Scheduler installer, a compatible version of Java 11 must be present on the target machine.
...
4. Check the **I accept the terms of the License Agreement** box, then select **Next**

5. On the **Select Install Location** screen, you can provide an alternative installation folder. We recommend leaving the default one as-is. Select **Next**.

![Installation destination folder](/img/legacy/installation-destination-folder-2.png)

6. The **Select Components** screen displays the required components that will be installed. JRE can be optionally unchecked if you prefer using an external installation of Java 11, however, we recommend leaving the default settings as-is. Select **Install**.
...
7. Select **Next** then **Finish** to close the wizard.
...
To continue with the Reports Scheduler configuration, see [this guide.](./openlm-reports-scheduler-configuration)
