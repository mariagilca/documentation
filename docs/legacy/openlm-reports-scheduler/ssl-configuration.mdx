...
## Overview
> This document explains how to activate and configure SSL (HTTPS) for the OpenLM Reports Scheduler component.
...
> Note: Some .pfx files can't be read by a certain version of Java. Change the version accordingly.
...
To update the Identity Service for SSL:
...
To configure the Java keystore:
...
5. Locate the OpenLM SLM folder and open the params.js file in a text editor (typically located at "C:\Program Files\OpenLM\OpenLM Server\bin\wwwroot\params.js).
...
8. Open Windows Services and restart the "OpenLM SLM" service.



## If OpenLM SLM is SSL secured

When the OpenLM SLM already uses SSL, update the Report Scheduler to match:
