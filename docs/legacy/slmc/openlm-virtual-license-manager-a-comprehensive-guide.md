...
- Add a description to the Device or User. As the user is shown only in the encrypted way by Device ID, a user with Manager Role cannot know or remember for the future who is that user, thus user with Manager Role can add a note or description of that Device ID (user) on time of allocation of a license, after being able to manage them by special page, reaching that page by selecting to several Devices Ids of a certain feature of Virtual Pool.
