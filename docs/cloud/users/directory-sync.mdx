...
Open the OpenLM Platform **app launcher** (grid icon, top-right) and select **Users → Directory Sync**.

**Before you start:** Install a Directory Sync Agent (DSA) on your network and generate an authorization file so it can authenticate with the **Identity** service, then approve the agent under **Agent Manager**.

**Related:** [Identity](/cloud/openlm-administration/identity) · [Users and Groups](/cloud/users/users-and-groups)
:::
