...
- **DSA** must be installed on at least one machine in your network.
- You must [generate an authorization file](../services/identity#generate-a-new-authorization-file) to allow the DSA to authenticate with the Identity Service.
...
:::warning
If DSS is still integrated with the OpenLM Server, be aware that the server clears all user properties—except **Mobile Phone**, **Email**, and **Country**—before updating data via LDAP sync.  
