...
All requests require a valid `Authorization: Bearer <token>` header. The Reporting Data API is a GraphQL API. All data is read through a single endpoint.
...
| `after` | String | (Cursor paging) A cursor string from a previous response that fetches the next page after it. |
...
These return a paged list of distinct, sorted values for one field, for building filter dropdowns. Every query takes an optional `searchTerm` plus the offset-paging parameters `skip` (Int) and `take` (Int). Each table lists the `searchTerm` type for that query.
